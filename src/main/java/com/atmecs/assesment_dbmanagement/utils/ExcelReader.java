...
public class ExcelReader {
	...
	public int getRowNumber(String path, int sheetIndex,String testcaseid) throws IOException {
		File file = new File(path);
		FileInputStream fileInput = new FileInputStream(file);
		XSSFWorkbook excelbook = new XSSFWorkbook(fileInput);
		Sheet excelsheet = excelbook.getSheetAt(sheetIndex);
	...
	public String getCellData(String path, int sheetIndex,String testcaseid,String columnname) throws IOException {
		File file = new File(path);
		FileInputStream fileInput = new FileInputStream(file);
		XSSFWorkbook excelbook = new XSSFWorkbook(fileInput);
		Sheet excelsheet = excelbook.getSheetAt(sheetIndex);
		...
		Iterator<Row> rows=excelsheet.rowIterator();
		int rownumber=getRowNumber(path, sheetIndex, testcaseid);
		int columnnumber=getColumnNumber(path,sheetIndex,columnname);
		while(rows.hasNext()) 
		{
			Row row=rows.next();
	...
	public int getColumnNumber(String path, int sheetIndex,String columnname) throws IOException {
		File file = new File(path);
		FileInputStream fileInput = new FileInputStream(file);
		XSSFWorkbook excelbook = new XSSFWorkbook(fileInput);
		Sheet excelsheet = excelbook.getSheetAt(sheetIndex);
